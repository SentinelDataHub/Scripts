...
grep '<id>' "$PWD/query-result" | tail -n +2 | cut -f2 -d'>' | cut -f1 -d'<' | cat -n > .product_id_list
grep '<title>' "$PWD/query-result" | tail -n +2 | cut -f2 -d'>' | cut -f1 -d'<' | cat -n > .product_title_list
sort -nk 1  .product_id_list .product_title_list | sed 's/[",:]/ /g' > product_list
